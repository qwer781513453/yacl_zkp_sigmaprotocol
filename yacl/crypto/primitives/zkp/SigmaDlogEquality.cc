...
void DlogEqualityProver::ComputeFirstMessage() {
  ...
  EC_POINT_mul(params.getGroup(), Msg.T1, nullptr, params.getG(), r,
               ctx);  // T1 = w2*G
  EC_POINT_mul(params.getGroup(), Msg.T2, nullptr, params.getH(), r,
               ctx);  // T2 = w2*H
...
BIGNUM* DlogEqualityGetChallenge(const DlogEqualityCommonInput& params,
  ...
  length[2] = EC_POINT_point2buf(params.getGroup(), params.getY1(),
                                 POINT_CONVERSION_COMPRESSED, &data[2], ctx);
  length[3] = EC_POINT_point2buf(params.getGroup(), params.getY2(),
